...
interface ZoomLightingColorPickerProps {
...
export default function ZoomLightingColorPicker({
  selectedColor,
  colors,
  onColorChange,
  onCustomColorChange,
  onFullscreen,
}: ZoomLightingColorPickerProps) {
  ...
  const handleColorClick = (color: any) => {
    ...
    // white-noise已被删除，跳转到白屏页面
    if (color.value === "white-noise") {
      router.push("/");
      return;
    }
    // 如果是zoom-lighting，保持在当前页面
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 transition-colors">
      ...
      <div className="text-center">
        ...
        <button
          className="inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium h-11 rounded-md px-8 w-full bg-orange-600 hover:bg-orange-700 dark:bg-orange-500 dark:hover:bg-orange-600 text-white"
