...
import BlueScreenHero from "./BlueScreenHero";
import BlueScreenInfo from "./BlueScreenInfo";
...
export default function BlueScreenClient() {
  ...
  const handleFullscreen = async () => {
    if (!isFullscreen) {
      ...
      fullscreenElement.id = "bluescreen-fullscreen";
  ...
  const handleDownload = () => {
    ...
    if (ctx) {
      ...
      link.download = `blue-page-${Date.now()}.png`;
  ...
  const handleDownload4K = () => {
    ...
    if (ctx) {
      ...
      const link = document.createElement("a");
      link.download = `blue-page-4k-${Date.now()}.png`;
      link.href = canvas.toDataURL();
      link.click();
    }
  };
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      // 如果没有全屏元素，清理创建的全屏元素
      if (!isInFullscreen) {
        const fullscreenElement = document.getElementById("bluescreen-fullscreen");
