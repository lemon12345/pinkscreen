...
import WhiteScreenHero from "./WhiteScreenHero";
...
export default function WhiteScreenClient() {
  ...
  useEffect(() => {
    ...
    const handleFullscreenChange = () => {
      if (!document.fullscreenElement && !(document as any).webkitFullscreenElement &&
        ...
        // 移除全屏预览元素
        const fullscreenElement = document.getElementById("whitescreen-fullscreen");
  ...
  const handleFullscreen = async () => {
    if (!isFullscreen) {
      ...
      fullscreenElement.id = "whitescreen-fullscreen";
  ...
  const handleDownload = () => {
    ...
    if (ctx) {
      ...
      link.download = `white-page-${Date.now()}.png`;
  ...
  const handleDownload4K = () => {
    ...
    if (ctx) {
      ...
      const link = document.createElement("a");
      link.download = `white-page-4k-${Date.now()}.png`;
      link.href = canvas.toDataURL();
      link.click();
    }
  };
