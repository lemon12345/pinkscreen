...
import PurpleScreenHero from "./PurpleScreenHero";
import PurpleScreenInfo from "./PurpleScreenInfo";
...
export default function PurpleScreenClient() {
  ...
  const handleFullscreen = async () => {
    if (!isFullscreen) {
      ...
      fullscreenElement.id = "purplescreen-fullscreen";
  ...
  const handleDownload = () => {
    ...
    if (ctx) {
      ...
      link.download = `purple-page-${Date.now()}.png`;
  ...
  const handleDownload4K = () => {
    ...
    if (ctx) {
      ...
      const link = document.createElement("a");
      link.download = `purple-page-4k-${Date.now()}.png`;
      link.href = canvas.toDataURL();
      link.click();
    }
  };
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      // 如果没有全屏元素，清理创建的全屏元素
      if (!isInFullscreen) {
        const fullscreenElement = document.getElementById("purplescreen-fullscreen");
