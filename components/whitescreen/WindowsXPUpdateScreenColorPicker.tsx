...
interface WindowsXPUpdateScreenColorPickerProps {
...
export default function WindowsXPUpdateScreenColorPicker({
  selectedColor,
  colors,
  onColorChange,
  onCustomColorChange,
  ...
  onStartTimeChange,
}: WindowsXPUpdateScreenColorPickerProps) {
  ...
  const handleRestart = () => {
    console.log('WindowsXPUpdateScreenColorPicker: handleRestart called');
    // Clear existing interval
    if (updateIntervalRef.current) {
      clearInterval(updateIntervalRef.current);
    }
    ...
    // 调用父组件的onRestart回调
    if (onRestart) {
      console.log('WindowsXPUpdateScreenColorPicker: calling onRestart callback');
      onRestart();
    } else {
      console.log('WindowsXPUpdateScreenColorPicker: onRestart callback is undefined');
    }
  };

  // 添加调试信息
  useEffect(() => {
    console.log('WindowsXPUpdateScreenColorPicker: updateDuration changed to', updateDuration);
  }, [updateDuration]);

  useEffect(() => {
    console.log('WindowsXPUpdateScreenColorPicker: startTime changed to', startTime);
  }, [startTime]);
            ...
            <button
              ...
              style={{
                ...
                backgroundColor: screen.value === "windows-10" ? "#0078d7" :
                  screen.value === "windows-xp" ? "#5a7edc" :
      ...
      <div className="mt-6">
        <div className="flex space-x-4 mb-4">
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t("colorPicker.updateTime")}</label>
            <div className="flex items-center space-x-2">
              ...
              />
              <span className="text-sm text-gray-600 dark:text-gray-400">{t("colorPicker.minutes")}</span>
            </div>
          </div>
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t("colorPicker.startTime")}</label>
            <div className="flex items-center space-x-2">
              <input
                type="number"
                min="0"
