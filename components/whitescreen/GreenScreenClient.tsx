...
import GreenScreenHero from "./GreenScreenHero";
import GreenScreenInfo from "./GreenScreenInfo";
import GreenScreenUseCases from "./GreenScreenUseCases";
...
export default function GreenScreenClient() {
  ...
  const handleFullscreen = async () => {
    if (!isFullscreen) {
      ...
      fullscreenElement.id = "greenscreen-fullscreen";
  ...
  const handleDownload = () => {
    ...
    if (ctx) {
      ...
      link.download = `green-page-${Date.now()}.png`;
  ...
  const handleDownload4K = () => {
    ...
    if (ctx) {
      ...
      const link = document.createElement("a");
      link.download = `green-page-4k-${Date.now()}.png`;
      link.href = canvas.toDataURL();
      link.click();
    }
  };
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      // 如果没有全屏元素，清理创建的全屏元素
      if (!isInFullscreen) {
        const fullscreenElement = document.getElementById("greenscreen-fullscreen");
