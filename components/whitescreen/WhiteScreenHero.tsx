...
interface WhiteScreenHeroProps {
...
const COLORS_BACKGROUND = ["#FFFFFF", "#F8F9FA", "#F1F3F4", "#E8EAED"];

export default function WhiteScreenHero({
  selectedColor,
  onColorChange,
  onFullscreen,
  onDownload,
  onCustomColorChange,
}: WhiteScreenHeroProps) {
  ...
  const backgroundImage = useMotionTemplate`radial-gradient(125% 125% at 50% 0%, #ffffff 0%, #f8fafc 50%, #f1f5f9 100%)`;
  ...
  return (
    <motion.section
      className="relative grid place-content-center overflow-hidden py-16 md:py-28 2xl:py-40 bg-gradient-to-br from-indigo-50 via-cyan-50 to-blue-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950"
    >
      {/* Background decorative elements */}
      <div className="absolute inset-0">
        <div className="absolute top-20 left-20 w-32 h-32 bg-blue-400/20 dark:bg-blue-300/30 rounded-full blur-3xl animate-pulse" />
        <div className="absolute top-40 right-20 w-40 h-40 bg-purple-400/20 dark:bg-purple-300/30 rounded-full blur-3xl animate-bounce" />
        <div className="absolute bottom-20 left-1/3 w-28 h-28 bg-pink-400/20 dark:bg-pink-300/30 rounded-full blur-2xl animate-pulse delay-1000" />
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center px-4">
        ...
        >
          <motion.span
            className="inline-flex items-center gap-2 rounded-full bg-gray-700/60 dark:bg-gray-200/30 px-3 py-1.5 text-sm text-white dark:text-gray-900 font-medium"
        ...
        >
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              <Monitor className="w-5 h-5 text-gray-800 dark:text-white" />
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t("interface.livePreview")}</h3>
            </div>
            ...
            <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2 bg-black/20 text-white px-4 py-2 rounded-full backdrop-blur-sm">
                  ...
                  <span className="text-sm">{t("interface.clickForFullscreen")}</span>
                </div>
              </div>
            ...
            <div className="text-sm text-gray-900 dark:text-white font-semibold drop-shadow-sm">
              {t("interface.current")}: {selectedColor}
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              <Sparkles className="w-5 h-5 text-gray-800 dark:text-white" />
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">{t("interface.chooseYourColor")}</h3>
            </div>
              ...
              {colors.map((colorOption) => (
                colorOption.href ? (
                  ...
                  >
                    ...
                    <div className="text-xs text-center mt-1 text-gray-900 dark:text-white font-semibold tracking-wide drop-shadow-sm">
                      {colorOption.name}
                    </div>
                  </I18nLink>
                ) : (
            ...
            <p className="text-sm text-gray-900 dark:text-white font-semibold drop-shadow-sm">
              {t("interface.perfectFor")}
            </p>
          </motion.div>
        </motion.div>
