...
interface Windows11UpdateScreenColorPickerProps {
...
export default function Windows11UpdateScreenColorPicker({
  selectedColor,
  colors,
  onColorChange,
  onCustomColorChange,
  ...
  onStartTimeChange,
}: Windows11UpdateScreenColorPickerProps) {
  ...
  const handleRestart = () => {
    console.log('Windows11UpdateScreenColorPicker: handleRestart called');
    // Clear existing interval
    if (updateIntervalRef.current) {
      clearInterval(updateIntervalRef.current);
    }
    ...
    // 调用父组件的onRestart回调
    if (onRestart) {
      console.log('Windows11UpdateScreenColorPicker: calling onRestart callback');
      onRestart();
    } else {
      console.log('Windows11UpdateScreenColorPicker: onRestart callback is undefined');
    }
  };

  // 添加调试信息
  useEffect(() => {
    console.log('Windows11UpdateScreenColorPicker: updateDuration changed to', updateDuration);
  }, [updateDuration]);

  useEffect(() => {
    console.log('Windows11UpdateScreenColorPicker: startTime changed to', startTime);
  }, [startTime]);
