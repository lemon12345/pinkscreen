...
import BlackScreenHero from "./BlackScreenHero";
import BlackScreenInfo from "./BlackScreenInfo";
...
export default function BlackScreenClient() {
  ...
  const handleFullscreen = async () => {
    if (!isFullscreen) {
      ...
      fullscreenElement.id = "blackscreen-fullscreen";
  ...
  const handleDownload = () => {
    ...
    if (ctx) {
      ...
      link.download = `black-page-${Date.now()}.png`;
  ...
  const handleDownload4K = () => {
    ...
    if (ctx) {
      ...
      const link = document.createElement("a");
      link.download = `black-page-4k-${Date.now()}.png`;
      link.href = canvas.toDataURL();
      link.click();
    }
  };
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      // 如果没有全屏元素，清理创建的全屏元素
      if (!isInFullscreen) {
        const fullscreenElement = document.getElementById("blackscreen-fullscreen");
