...
interface WhiteScreenColorPickerProps {
...
export default function WhiteScreenColorPicker({
  selectedColor,
  colors,
  onColorChange,
  onCustomColorChange,
  onFullscreen,
}: WhiteScreenColorPickerProps) {
  const t = useTranslations("WhiteScreen");
  const router = useRouter();

  const handleColorClick = (color: { name: string; value: string; className: string }) => {
    // 如果是红色，跳转到红色屏幕页面
